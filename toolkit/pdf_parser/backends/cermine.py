...
class Cermine(Backend):
    def __init__(self):
        ...
        self.logger = logging.getLogger('pdf_parser.backend.cermine')
    ...
    def _process_pdf(self, input_file, output_dir, service, **kwargs):
        ...
        with tempfile.TemporaryDirectory() as dirname:
            ...
            try:
                ...
                self.logger.warning('Cermine exit exceptly with error: %s', str(e))
    ...
    def _process_batch(self, batch_input_files, output_dir, service, **kwargs):
        if not self.health:
            return 0
        self.logger.debug('One Batch of %s pdfs: %s...', len(batch_input_files), ', '.join(list(batch_input_files)[:3]))
        ...
        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            ...
            for future in as_completed(futures):
                ...
                if count // 1000 != (count - future.result()) // 1000:
                    self.logger.debug(f'{count} PDF files are processed')
                if count // 10000 != (count - future.result()) // 10000:
                    self.logger.info(f'{count} PDF files are processed')
