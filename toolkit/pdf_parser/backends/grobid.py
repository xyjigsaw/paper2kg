...
class Grobid(Backend):
    def __init__(self, host='10.10.10.10', port=8074, sleep_time=5, coordinates=None):
        ...
        self.logger = logging.getLogger('pdf_parser.backend.grobid')
    ...
    def _check_server(self):
        ...
        try:
            ...
            self.health = False
            self.logger.error(f'Cannot connect to {self.url}.')
            return

        status = r.status_code
        if status == 200:
            self.health = True
            self.logger.info('GROBID server is up and running.')
        else:
            self.health = False
            self.logger.error('GROBID server does not appear.')
    ...
    def _process_pdf(self, input_file, output_dir, service, **kwargs):
        ...
        while True:
            ...
            if r.status_code == 200:  # success
                ...
                self.logger.warning(f"PDF parse failed: {input_file} with http code {r.status_code}")
                return 0
        ...
        try:
            ...
            self.logger.error(f"Could not write out result file: {output_file}")
            return 0
    ...
    def _process_dir(self, input_dir, output_dir, service, n_threads=0, **kwargs):
        ...
        self.logger.info(f'{len(pdf_files)} PDF files to process.')
        with ThreadPoolExecutor(max_workers=n_threads) as executor:
            ...
            for idx, _ in enumerate(as_completed(futures)):
                if idx % 1000 == 0:
                    self.logger.debug(f'{idx} PDF files are processed')
                if idx % 10000 == 0:
                    self.logger.info(f'{idx} PDF files are processed')
